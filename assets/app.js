...
(function () {
  ...
  (function corridorSim() {
    ...
    function neighborsOf(idx) {
      ...
      if (idx < poleEls.length - 1) arr.push(idx + 1);
      return arr;
    }
    ...
    function triggerPole(idx) {
      ...
      setTimeout(function () {
        ...
        logEl.innerHTML = "<strong>Pole " + (idx + 1) + "</strong>: sharing alert with nearby poles…";
        neighbors.forEach(function (n) {
          var sigIdx = Math.min(n, idx);
          if (signalEls[sigIdx]) signalEls[sigIdx].classList.add("on");
        });
      }, 1200);
      ...
      setTimeout(function () {
        neighbors.forEach(function (n) {
          var sigIdx = Math.min(n, idx);
          if (signalEls[sigIdx]) signalEls[sigIdx].classList.remove("on");
        });
        ...
        setStatus(idx, "WARN");
        neighbors.forEach(function (n) {
          poleEls[n].classList.add("active");
          setStatus(n, "WARN");
        });
        ...
        logEl.innerHTML = "<strong>Beacon active</strong> — approaching drivers warned.";
      }, 1800);
      ...
      setTimeout(function () {
        ...
        setStatus(idx, "IDLE");
        neighbors.forEach(function (n) {
          poleEls[n].classList.remove("active", "busy");
          setStatus(n, "IDLE");
        });
    ...
    function tick(ts) {
      ...
      var near = false;
      POLE_POS.forEach(function (pos, i) {
        if (poleEls[i].classList.contains("active") && Math.abs(pct - pos) < 6) near = true;
      });
